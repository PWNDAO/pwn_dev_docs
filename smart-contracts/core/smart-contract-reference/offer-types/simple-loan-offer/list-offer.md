...
* `Offer calldata`**`offer`** - Offer struct with the list offer parameters
...
* `Offer memory`**`Offer`** - List offer struct to encode
* `OfferValues memory`**`offerValues`** - Concrete values for list offer from a borrower.
...
Returns a list offer hash according to [EIP-712](https://eips.ethereum.org/EIPS/eip-712).

This function takes one argument supplied by the caller:

* `Offer memory`**`offer`** - List offer struct to get hash of
